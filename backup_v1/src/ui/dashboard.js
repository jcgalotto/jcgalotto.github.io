...
export function updateKPIs(monthlyData, usdAmount) {
    ...
    // Update KPI values
    document.getElementById('kpiVariation').textContent = `${variation >= 0 ? '+' : ''}${variation.toFixed(2)}%`;
    const kpiStatus = document.getElementById('kpiStatus');
    kpiStatus.textContent = variation >= 0 ? '📈 Ganancia' : '📉 Pérdida';
    kpiStatus.className = variation >= 0 ? 'kpi-status text-success' : 'kpi-status text-danger';

    document.getElementById('kpiInitial').textContent = `Gs. ${formatNumber(initialPYG.toFixed(2))}`;
    document.getElementById('kpiReal').textContent = `Gs. ${formatNumber(finalRealPYG.toFixed(2))}`;
}
